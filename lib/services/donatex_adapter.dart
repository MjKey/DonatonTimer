...
class DonateXAdapter extends BaseDonationServiceAdapter {
  static const String _negotiateUrl =
      'https://donatex.gg/api/controls-hub/negotiate?negotiateVersion=1';
    ...
    try {
      ...
      _subscription = _channel!.stream.listen(
        ...
        onDone: () {
          ...
          LogManager.warning(
            'DonateX: соединение закрыто (code: $closeCode, reason: $closeReason)',
          );
  ...
  void _sendPing() {
    // Initial SignalR ping (type: 6) right after handshake ack.
  ...
  void _joinWidgetGroup() {
    ...
    final joinMsg = jsonEncode({
      'arguments': [_widgetId],
      'invocationId': _nextInvocationId.toString(),
  ...
  void _sendHeartbeat() {
    ...
    _nextInvocationId++;
    _channel?.sink.add('$heartbeat$_recordSeparator');
  }
  ...
  void _processSignalRMessage(Map<String, dynamic> json) {
    ...
    if (type == 1 &&
        (target == 'ServerTimeSync' || target == 'RefreshWidget')) {
      return;
    }
    ...
    LogManager.info(
      'DonateX: получено: ${logData.length > 200 ? '${logData.substring(0, 200)}...' : logData}',
    );
  }
  ...
  void _scheduleReconnect() {
    ...
    _reconnectTimer = Timer(
      ...
      () async {
        _reconnectAttempts++;
        LogManager.info(
          'DonateX: попытка переподключения #$_reconnectAttempts',
        );
        updateStatus(ConnectionStatus.reconnecting);
